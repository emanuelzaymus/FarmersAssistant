...
namespace structures
{
	...
	{
		TreeNode<T>* newNode = shallowCopy();
		for (size_t i = 0; i < newNode->degree(); i++)
		{
			TreeNode<T>* son = newNode->getSon(i);

			if (son != nullptr)
				son->replaceSon(son->deepCopy(), i);
		}
		return newNode;
	}
	...
	{
		if (parent_ == nullptr)
			throw std::exception("No brother - I am root!");
