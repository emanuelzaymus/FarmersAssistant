...
namespace structures
{
	...
	inline BinarySearchTree<K, T>& BinarySearchTree<K, T>::operator=(const BinarySearchTree<K, T>& other)
	{
		...
		{
			clear();
			Tree<TableItem<K, T>*>::PreOrderTreeIterator iterB(other.binaryTree_->getRoot());
			Tree<TableItem<K, T>*>::PreOrderTreeIterator iterE(nullptr);
			while (iterB != iterE)
			{
				TableItem<K, T>* item = *iterB;
				++iterB;
	...
	{
		typename BSTTreeNode* newNode = new BinaryTreeNode<TableItem<K, T>*>(new TableItem<K, T>(key, data));
		if (!tryToInsertNode(newNode))
		{
			delete newNode->accessData();
			delete newNode;
			throw std::logic_error("Key is already in.");
		}
	}
	...
	inline T BinarySearchTree<K, T>::remove(const K & key)
	{
		...
		if (found)
		{
			...
			throw std::logic_error("No such key in.");
		}
	}
	...
	{
		for each (auto item in *binaryTree_)
		{
			delete item;
		}
		size_ = 0;
	...
	{
		BSTTreeNode* node = (BSTTreeNode*)(binaryTree_->getRoot());
		BSTTreeNode* wanted = node;
	...
	inline bool BinarySearchTree<K, T>::tryToInsertNode(BSTTreeNode* node)
	{
		...
		if (binaryTree_->isEmpty())
		{
			...
			typename BSTTreeNode* parent = findBSTNode(node->accessData()->getKey(), found);
			if (!found)
			{
